...
import { AiStage, type Slider, type TerraformConfig, TerraformTarget, ControlConfig, SoundConfig, Modulation, ModulationSource, ModulationTarget, CameraData, ViewMode, ShipConfig, ShipModulation, ShipModulationTarget, Alien, Laser, Particle } from '../types';
...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const alienLasersRef = useRef<Laser[]>([]);
  const particlesRef = useRef<Particle[]>([]);
  const screenShakeRef = useRef<number>(0);
  const damageFlashRef = useRef<number>(0);
  const gunCooldownRef = useRef<number>(0);
  const triggerFireRef = useRef<boolean>(false);
  ...
  const handleRestartGame = useCallback(() => {
      ...
      gunCooldownRef.current = 0;
      triggerFireRef.current = false;

      // Teleport player back to initial position safely
      cameraRef.current.position = [0, 4, 15]; // Match INITIAL_CAMERA_POS from code
      cameraRef.current.rotation = [0, 0];
  ...
  useEffect(() => {
      const handleMouseDown = (e: MouseEvent) => {
          if ((e.target as HTMLElement).matches('button, input, textarea, select, a, svg, path')) return;
  ...
  const playLaserShootSound = useCallback(() => {
      const ctx = audioContextRef.current;
      const nodes = audioNodesRef.current;
      const cfg = soundConfigRef.current;
      if (!ctx || !nodes || !cfg.enabled) return;
  ...
  const playLaserHitSound = useCallback(() => {
      const ctx = audioContextRef.current;
      const nodes = audioNodesRef.current;
      const cfg = soundConfigRef.current;
      if (!ctx || !nodes || !cfg.enabled) return;
      ...
      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0.2 * cfg.masterVolume, now);
  ...
  const playExplosionSound = useCallback(() => {
      ...
      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0.5 * cfg.masterVolume, now);
      ...
      if (nodes.reverb) {
          gain.connect(nodes.reverb.input);
      }
  ...
  const playPlayerHitSound = useCallback(() => {
      ...
      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0.6 * cfg.masterVolume, now);
      ...
      osc.start(now);
      osc.stop(now + 0.3);
      setTimeout(() => { osc.disconnect(); gain.disconnect(); }, 350);
  }, []);
    ...
    const gameLoop = (timestamp: number) => {
        ...
        if (!isGameOverRef.current) {
            ...
            if (firePressed && gunCooldownRef.current <= 0) {
                ...
                // Play firing sound
                audioGeneratorsRef.current.playLaserShootSound();

                // Forward & Right vectors derived from rotation
                const fwdX = Math.sin(y) * Math.cos(p - FLIGHT_PITCH_OFFSET);
                const fwdY = -Math.sin(p - FLIGHT_PITCH_OFFSET);
                const fwdZ = Math.cos(y) * Math.cos(p - FLIGHT_PITCH_OFFSET);
                const rX = Math.cos(y);
                const rZ = -Math.sin(y);

                const laserSpeed = 16.0;

                // Left Cannon Laser Start Position
                playerLasersRef.current.push({
                    id: uuidv4(),
                    position: [
                        currentPos[0] + fwdX * 0.6 - rX * 0.4,
                        currentPos[1] + fwdY * 0.6,
                        currentPos[2] + fwdZ * 0.6 - rZ * 0.4
                    ],
                    velocity: [fwdX * laserSpeed, fwdY * laserSpeed, fwdZ * laserSpeed],
                    color: '#00ffff',
                    owner: 'player',
                    damage: 25,
                    size: 0.15,
                ...
                // Right Cannon Laser Start Position
                playerLasersRef.current.push({
                    id: uuidv4(),
                    position: [
                        currentPos[0] + fwdX * 0.6 + rX * 0.4,
        ...
        // --- SPAWN AND UPDATE ENEMIES (ALIENS) ---
        if (!isGameOverRef.current && aliensRef.current.length < 6) {
            // Spawn alien at random position orbiting the player
            const angle = Math.random() * Math.PI * 2;
            const dist = 12.0 + Math.random() * 8.0; // Spawns 12-20 units away
            const spawnX = currentPos[0] + Math.sin(angle) * dist;
            const spawnZ = currentPos[2] + Math.cos(angle) * dist;
            const spawnY = currentPos[1] + (Math.random() - 0.5) * 4.0;

            const alienTypes: ('scout' | 'drone' | 'boss')[] = ['scout', 'drone', 'scout', 'drone'];
            if (Math.random() > 0.85) alienTypes.push('boss');
            ...
            aliensRef.current.push({
                ...
                type,
                color,
                orbitRadius: 7.0 + Math.random() * 5.0,
                orbitSpeed,
                phase: Math.random() * Math.PI * 2,
                phaseY: Math.random() * Math.PI * 2,
                lastShootTime: timestamp + Math.random() * 1000,
                shootInterval
            });
        }
        ...
        for (let i = 0; i < aliensRef.current.length; i++) {
            ...
            a.velocity[0] += (targetVX - a.velocity[0]) * 0.08;
            a.velocity[1] += (targetVY - a.velocity[1]) * 0.08;
            a.velocity[2] += (targetVZ - a.velocity[2]) * 0.08;
            ...
            // Alien Shooting AI
            if (!isGameOverRef.current && (timestamp - a.lastShootTime) > a.shootInterval * 1000) {
                a.lastShootTime = timestamp;

                // Aim at player with standard error offset for dodge potential
                const vDirX = dx / dPlayer + (Math.random() - 0.5) * 0.12;
                const vDirY = dy / dPlayer + (Math.random() - 0.5) * 0.12;
                ...
                alienLasersRef.current.push({
                    ...
                    velocity: [(vDirX / len) * alSpeed, (vDirY / len) * alSpeed, (vDirZ / len) * alSpeed],
                    color: '#ff3c00', // Crimson orange energy ball
                    owner: 'alien',
                    damage: alDamage,
                    size: 0.12,
                    timeToLive: 4.5
        ...
        // --- UPDATE LASER ENTITIES & COLLISION RESOLUTIONS ---
        // Player Lasers
        for (let i = playerLasersRef.current.length - 1; i >= 0; i--) {
            const l = playerLasersRef.current[i];
            l.position[0] += l.velocity[0] * dt;
            l.position[1] += l.velocity[1] * dt;
            l.position[2] += l.velocity[2] * dt;
            l.timeToLive -= dt;

            let laserHit = false;
            for (let j = aliensRef.current.length - 1; j >= 0; j--) {
                const a = aliensRef.current[j];
                const lx = l.position[0] - a.position[0];
                const ly = l.position[1] - a.position[1];
                const lz = l.position[2] - a.position[2];
                ...
                if (dist < a.size * 1.5) {
                    ...
                    for (let pIdx = 0; pIdx < 6; pIdx++) {
                        particlesRef.current.push({
                            ...
                            timeToLive: 0.25 + Math.random() * 0.2,
                            maxLife: 0.45
                        });
                    }

                    // Play hitting sound
                    audioGeneratorsRef.current.playLaserHitSound();

                    if (a.health <= 0) {
                        // Play massive explosion sound
                        audioGeneratorsRef.current.playExplosionSound();
                        
                        // Fireball visual particles
                        const pCount = a.type === 'boss' ? 35 : a.type === 'drone' ? 20 : 10;
                        const explosionColor = a.color;
                        for (let pIdx = 0; pIdx < pCount; pIdx++) {
                            particlesRef.current.push({
                                ...
                                timeToLive: 0.45 + Math.random() * 0.45,
                                maxLife: 0.9
                            });
                        }
            ...
            if (laserHit || l.timeToLive <= 0) {
                playerLasersRef.current.splice(i, 1);
            }
        }

        // Alien Lasers
        for (let i = alienLasersRef.current.length - 1; i >= 0; i--) {
            const l = alienLasersRef.current[i];
            l.position[0] += l.velocity[0] * dt;
            l.position[1] += l.velocity[1] * dt;
            l.position[2] += l.velocity[2] * dt;
            l.timeToLive -= dt;

            // Check distance to player cockpit camera
            const px = l.position[0] - currentPos[0];
            const py = l.position[1] - currentPos[1];
            const pz = l.position[2] - currentPos[2];
            const dist = Math.sqrt(px*px + py*py + pz*pz);

            if (dist < 0.85) {
                // Hit player ship overlay
                if (!isGameOverRef.current) {
                    playerHealthRef.current = Math.max(0, playerHealthRef.current - l.damage);
                    ...
                    audioGeneratorsRef.current.playPlayerHitSound();

                    // Generate electrical impact sparks
                    for (let pIdx = 0; pIdx < 8; pIdx++) {
                        particlesRef.current.push({
                            ...
                            timeToLive: 0.3 + Math.random() * 0.3,
                            maxLife: 0.6
                        });
                    }
                    ...
                    if (playerHealthRef.current <= 0) {
                        ...
                        // Ship massive blue-green implosion
                        for (let pIdx = 0; pIdx < 50; pIdx++) {
                            particlesRef.current.push({
                                id: uuidv4(),
                                ...
                                timeToLive: 0.9 + Math.random() * 0.9,
                                maxLife: 1.8
                            });
                        }
                    }
                }
                alienLasersRef.current.splice(i, 1);
            } else if (l.timeToLive <= 0) {
                alienLasersRef.current.splice(i, 1);
            }
        }
        ...
        // --- FADE EFFECTS OVER TIME ---
        if (screenShakeRef.current > 0) {
            screenShakeRef.current = Math.max(0, screenShakeRef.current - dt * 3.2);
        }
        if (damageFlashRef.current > 0) {
            damageFlashRef.current = Math.max(0, damageFlashRef.current - dt * 4.5);
        }
  ...
  }, [playGenerativeNote, playArpNote, playRhythm, playCollisionSound, playLaserShootSound, playLaserHitSound, playExplosionSound, playPlayerHitSound]);
  ...
  return {
    ...
    playerHealth, playerMaxHealth, score, isGameOver, aliensRef, playerLasersRef, alienLasersRef, particlesRef, screenShakeRef, damageFlashRef, triggerFireRef, handleRestartGame
  };
};
